...
namespace DGLab {
    public class DGLab_BLE {
        ...
        public void SetLevel(Channel ch, int val) {
            var power = ReadBytes(level);
            if(ch == Channel.A) {
                power[1] = (byte)((val << 3) | (power[1] & 7));
                power[2] = (byte)(val >> 5);
            } else {
                power[0] = (byte)(val & 0xFF);
                power[1] = (byte)((val >> 8) & 0xFF);
            }
            //var val = ((a & 0x7FF) << 11) | (b & 0x7FF);
            //power[0] = (byte)(val & 0xFF);
            //power[1] = (byte)((val >> 8) & 0xFF);
            //power[2] = (byte)((val >> 16) & 0xFF);
            WriteBytes(level, power);
        }
